...
//
//class Sampler(private val extractor: Extractor, private val dictionary: Set<String>) : TweetMaker {
...
//    companion object {
//
//        fun build(c: Config): Sampler {
//            val network = NetworkManager.load(
//                    c.networkFile,
//                    c.defaultCharacterMap
//            )
//
//            val data = FileLoader(listOf(Config.tweets), network.characterMap()).contents
//            val dictionary = data.map { l -> l.split(" ") }.flatten().map { w -> w.toLowerCase() }.toSet()
//
//            return Sampler(Extractor(network), dictionary)
//        }
//
//    }
//}
